...
class CryptoManager:
    ...
    def get_public_key(self) -> bytes:
        ...
        The protocol strictly expects a 160-byte payload for the public key.
        """
        ...
        # Ensure the public key is exactly 160 bytes as expected by the RequestParser.
        # If pycryptodome generates slightly fewer bytes, we pad it with nulls.
        if len(der_public_key) < 160:
            der_public_key = der_public_key.ljust(160, b'\x00')
        elif len(der_public_key) > 160:
            # If it exceeds 160, we truncate it. (Note: standard 1024-bit RSA DER
            # SubjectPublicKeyInfo is typically exactly 160-162 bytes).
            der_public_key = der_public_key[:160]

        return der_public_key
